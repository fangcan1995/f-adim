...
import { SeerTreeNode } from "../seer-tree/SeerTreeNode";
...
export class TreeNode implements ITreeNode {
    ...
    get canExpand() { return this.treeModel.canExpand(this) };
    get isActive() { return this.treeModel.isActive(this) };
    get isAllChildrenActive() { return this.treeModel.isAllChildrenActive(this) };
    ...
    get customIconField() {
        return this.getField('customIcon');
    }

    get forbidChangeTypeField() {
        return this.getField('forbidChangeType');
    }
    ...
    // helper methods:
    loadChildren() {
        this.fireEvent({ eventName: TREE_EVENTS.onLazyLoadChildren, node: this });
    }

    lazyLoadChildren(children: SeerTreeNode[]) {
    ...
    activate(ids: string[], expand = true) {
        if (ids.indexOf(this.id) >= 0) {
            this.setIsActive(true, true);
            if (expand) {
                this.setIsExpanded(true);
                this.expandParent(this);
            }
        }
        if (this.children) {
            this.children.forEach((child) => {
    ...
    expandParent(node: TreeNode) {
        if (node.realParent) {
            node.realParent.setIsExpanded(true);
            this.expandParent(node.realParent);
        }
    }
    ...
    addNewNode(newNodeData) {
        let newNode = new TreeNode(newNodeData, this, this.treeModel);
        this.children.push(newNode);
    }

    genNewNode(newNodeData) {
        return new TreeNode(newNodeData, this, this.treeModel);
    }
}
