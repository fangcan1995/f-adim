...
export class OrgService extends BaseService<any>{
  ...
  deleteOne (id): Promise<ResModel> {
    return this._httpInterceptorService.request('DELETE', `http://172.16.1.252:9080/organizations/staffs/${id}?`, {}, false).toPromise();
  }

  // 表格数据
 getData(params): Promise<any>{
   return this._httpInterceptorService.request('GET', `http://172.16.1.252:9080/organizations`, params, false).toPromise();
 }
  ...
  getOrganizationsById (orgId): Promise<ResModel> {
    return this._httpInterceptorService.request('GET', `http://172.16.1.252:9080/organizations/${orgId}?departmentId=${orgId}`, {}, false).toPromise();
  }

  getStaffsByOrgId(orgId): Promise<ResModel> {
    return this._httpInterceptorService.request('GET',`http://172.16.1.252:9080/organizations/staffs/${orgId}`,{}, false).toPromise();
  }

  operationRecord(data): Promise<ResModel> {
    return this._httpInterceptorService.request('GET',`http://172.16.1.252:9080/organizations/operation`, data).toPromise();
  }



  updateStaffOrgId(data): Promise<ResModel> {
    return this._httpInterceptorService.request('PUT',`http://172.16.1.252:9080/organizations/staff`, data).toPromise();
  }


  addOrganization(data): Promise<ResModel> {
    let url = `http://172.16.1.252:9080/organizations`;   //测试用地址
    console.log(data);
  ...
  delOrganization(orgId): Promise<ResModel> {
    let url = `http://172.16.1.252:9080/organizations/${orgId}`;   //测试用地址
    return this._httpInterceptorService.request('DELETE', url,{}).toPromise();
  }

  /*设置员工为组织机构的负责人*/
  configDepartLeader(params): Promise<ResModel> {
    let url = `http://172.16.1.252:9080/organizations/department/employerChangeLeader`;
  ...
  getStaffInfo (id): Promise<ResModel> {
    let url = `http://172.16.1.252:9080/staffs/${id}`;
