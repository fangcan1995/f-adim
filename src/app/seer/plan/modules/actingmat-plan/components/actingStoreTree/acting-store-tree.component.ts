...
@Component({
  selector: 'ActingStoreTreeComponent',
  templateUrl: './acting-store-tree.component.html',
  styleUrls: ['./acting-store-tree.component.scss'],
  providers: [Ng2Uploader],

})
export class ActingStoreTreeComponent extends BaseModalComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    if(this.data.treeType == 'add'){
      this.customerStoreId = this.data.storeTreeDataList;
    }else{
      if(this.data.storeTreeDataList.length == 0  && this.changeType!='change') {
        if(this.data.budget.customerStoreIds){
          this.customerStoreId = this.data.budget.customerStoreIds;
        }else{
  ...
  getStores(): void {
    ...
    this.gs.notifyDataChanged('getActingStores',this.storeData);
    this.seerTree.clearHistory();
    this.closeModal();
  }

  getStoreTreeData(){
    this.actingmatPlanService.getStoreTree().subscribe((result:Json)=>{
      if (result.success){
        result.data.forEach(v =>{
          if(v.id){
            this.customerStoreId.forEach(b =>{
                if(v.id == b){
                  v['selected'] = true;
                }
              }
