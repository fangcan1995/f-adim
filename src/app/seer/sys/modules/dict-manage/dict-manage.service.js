"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var core_1 = require("@angular/core");
var Observable_1 = require('rxjs/Observable');
var http_1 = require('@angular/http');
var http_2 = require('@angular/http');
var alertService = (function () {
    function alertService(http) {
        this.http = http;
        this.getAllAlertsUrl = 'http://localhost:8080/sys/alert/getAll';
        this.addAlertUrl = 'http://localhost:8080/sys/alert/addAlert';
        this.getAlertById = 'http://localhost:8080/sys/alert/';
        this.updateAlertUrl = 'http://localhost:8080/sys/alert/updateAlert';
        this.deleteByIdUrl = 'http://localhost:8080/sys/alert/deleteAlert';
        this.deleteSelectedAlertUrl = 'http://localhost:8080/sys/alert/deleteSelectedAlert';
    }
    alertService.prototype.extractData = function (res) {
        var body = res.json();
        return body || {};
    };
    alertService.prototype.handleError = function (error) {
        // In a real world app, we might use a remote logging infrastructure
        var errMsg;
        if (error instanceof http_1.Response) {
            var body = error.json() || '';
            var err = body.error || JSON.stringify(body);
            errMsg = error.status + " - " + (error.statusText || '') + " " + err;
        }
        else {
            errMsg = error.message ? error.message : error.toString();
        }
        console.error(errMsg);
        return Observable_1.Observable.throw(errMsg);
    };
    alertService.prototype.getAlerts = function () {
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' });
        var options = new http_2.RequestOptions({ headers: headers });
        return this.http.get(this.getAllAlertsUrl, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    alertService.prototype.addAlert = function (alert) {
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' });
        var options = new http_2.RequestOptions({ headers: headers });
        return this.http.post(this.addAlertUrl, alert, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    alertService.prototype.updateAlert = function (alert) {
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' });
        var options = new http_2.RequestOptions({ headers: headers });
        return this.http.post(this.updateAlertUrl, alert, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    alertService.prototype.removeAlert = function (id) {
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' });
        var options = new http_2.RequestOptions({ headers: headers });
        return this.http.get(this.deleteByIdUrl + "/" + id, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    alertService.prototype.removeAllSelectedAlerts = function (ids) {
        var headers = new http_2.Headers({ 'Content-Type': 'application/json' });
        var options = new http_2.RequestOptions({ headers: headers });
        return this.http.get(this.deleteSelectedAlertUrl + "/" + ids, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    alertService = __decorate([
        core_1.Injectable()
    ], alertService);
    return alertService;
}());
exports.alertService = alertService;
