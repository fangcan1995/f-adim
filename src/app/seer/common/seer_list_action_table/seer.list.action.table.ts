...
@Component({
  selector: 'seerlistactiontable',
  styleUrls: ['./seer.list.action.table.css'],
  templateUrl: './seer.list.action.table.html',

})
export class seerListActionTableComponent implements OnInit {
  ...
  openLink(event,key) {
    this.notify.emit({type: 'link', data: event,key:key });
  }
}
